      ...
      {tiles.map((tile) => {
        ...
        return (
          <Link
            key={tile.label}
            href={tile.href}
            className="group flex flex-col items-start rounded-lg border border-charcoal/10 bg-white p-6 transition-all duration-200 hover:-translate-y-1 hover:border-gold/40 hover:shadow-md"
          >
            <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-lg bg-maroon/[0.06] transition-colors group-hover:bg-maroon">
              <Icon className="h-5 w-5 stroke-maroon transition-colors group-hover:stroke-stone" />
            </div>
